        ...
        for k in range(0,7):
            ...
            for l in range(0,len(percent_cum[f][k])):
                if decide <= percent_cum[f][k][l]:
                    ...
                    break
        a = sell[f][final_choice[0]]
        b = trans2[f][final_choice[1]]
        c = store1[f][final_choice[2]]
        d = trans1[f][final_choice[3]]
        e_init=emission(f,a[0],a[1],a[2],a[3],b[0],b[1],b[2],c[0],c[1],c[2],c[3],1.6,0.1,d[0],d[1],d[2],d[3],final_choice[4],energy[final_choice[5]],disposal[final_choice[6]],package_emission[f])
        try_zoom = 0
        #其实可以做的：每个选项分别生成一组，
